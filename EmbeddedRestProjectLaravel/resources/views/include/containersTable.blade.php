...
<script>
    $("#containersGrid").jsGrid({
        ...
        controller: {
            loadData: function (filter) {
                ...
                $.ajax({
                    ...
                    url: "/api/v1/GET/containers",
                    dataType: "json"
                }).done(function(response){
                    var filteredResponse = $.grep(response, function(object) {
                        return (!filter.Container_ID || String(object.Container_ID).indexOf(filter.Container_ID) > -1)
                            && (!filter.Serienummer|| String(object.Serienummer).indexOf(filter.Serienummer) > -1)
                            && (!filter.Volume || String(object.Volume).indexOf(filter.Volume) > -1)
            ...
            updateItem: function(item) {
                return $.ajax({
                    ...
                    url: "/api/v1/POST/update/containers/"+item.Container_ID,
                    data: item
                });
            },
            insertItem: function (item) {
                ...
                return $.ajax({
                    ...
                    url: "/api/v1/POST/create/containers",
                    data: item
                }).done(function(response){
            ...
            deleteItem: function (item) {
                return $.ajax({
                    ...
                    url: "/api/v1/DELETE/containers/"+item.Container_ID          
                });
            }
        },
        ...
        deleteConfirm: "Do you really want to delete the Container?",
 
        fields: [
            { name: "Container_ID", type: "text", width: 150, inserting: false, editing: false},
            { name: "Serienummer", type: "text", width: 150 },
            { name: "Volume", type: "text", width: 200 },
            { name: "Van", type: "text", width: 200 },
            { name: "Tot", type: "text", width: 150 },
            { type: "control" }
        ]
    });
</script>
