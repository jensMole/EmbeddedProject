...
<script>
    $("#modulesGrid").jsGrid({
        ...
        controller: {
            loadData: function (filter) {
                ...
                $.ajax({
                    ...
                    url: "http://localhost:8000/api/v1/GET/modules",
                    dataType: "json"
                }).done(function(response){
                    var filteredResponse = $.grep(response, function(object) {
                        return (!filter.Module_ID || String(object.Module_ID).indexOf(filter.Module_ID) > -1)
                            && (!filter.Serienummer || String(object.Serienummer).indexOf(filter.Serienummer) > -1)
                            && (!filter.Fabrikant || String(object.Fabrikant).indexOf(filter.Fabrikant) > -1)
            ...
            updateItem: function(item) {
                return $.ajax({
                    ...
                    url: "/api/v1/POST/update/modules/"+item.Module_ID,
                    data: item
                });
            },
            insertItem: function (item) {
                ...
                return $.ajax({
                    ...
                    url: "/api/v1/POST/create/modules",
                    data: item
                }).done(function(response){
            ...
            deleteItem: function (item) {
                return $.ajax({
                    ...
                    url: "/api/v1/DELETE/modules/"+item.Module_ID       
                });
            }
        },
        ...
        fields: [
            { name: "Module_ID", type: "text", width: 150, inserting: false, editing: false },
            { name: "Serienummer", type: "text", width: 150 },
            { name: "Fabrikant", type: "text", width: 200 },
