<div id="verantwoordelijkenGrid"></div>
<script>
    $("#verantwoordelijkenGrid").jsGrid({
        width: "100%",
        margin: "0",
        ...
        controller: {
            loadData: function (filter) {
                ...
                $.ajax({
                    ...
                    url: "http://localhost:8000/api/v1/GET/verantwoordelijken",
                    dataType: "json"
                }).done(function(response){
                    var filteredResponse = $.grep(response, function(object) {
                        return (!filter.Verantwoordelijke_ID || String(object.Verantwoordelijke_ID).indexOf(filter.Verantwoordelijke_ID) > -1)
                            && (!filter.Naam || String(object.Naam).indexOf(filter.Naam) > -1)
            ...
            updateItem: function(item) {
                return $.ajax({
                    ...
                    url: "/api/v1/POST/update/verantwoordelijken/"+item.Verantwoordelijke_ID,
                    data: item
                });
            },
            insertItem: function (item) {
                ...
                return $.ajax({
                    ...
                    url: "/api/v1/POST/create/verantwoordelijken",
                    data: item
                }).done(function(response){
            ...
            deleteItem: function (item) {
                return $.ajax({
                    ...
                    url: "/api/v1/DELETE/verantwoordelijken/"+item.Verantwoordelijke_ID          
                });
            }
        },
        ...
        fields: [
            { name: "Verantwoordelijke_ID", type: "text", width: 150, inserting: false, editing: false },
            { name: "Naam", type: "text", width: 150 },
            { name: "Van", type: "text", width: 200 },
            { name: "Tot", type: "text", width: 200 },
            { type: "control" }
        ]
    });
</script>